    ...
    <div className="min-h-screen flex items-center justify-center bg-stone-50 relative overflow-hidden px-4">
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-4xl font-serif text-stone-900 text-center">Welcome Back.</h1>
          <p className="text-stone-500 font-sans font-light mt-2 text-center">Sign in to your serene financial dashboard.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block font-sans font-medium text-sm text-stone-700 mb-2" htmlFor="login-email">Email Address</label>
          ...
          <div>
            <label className="block font-sans font-medium text-sm text-stone-700 mb-2" htmlFor="login-password">Password</label>
          ...
          <button type="submit" disabled={loading} className="btn-elegant w-full py-4 rounded-xl text-lg font-medium mt-2 flex items-center justify-center">
            {loading ? <span className="w-5 h-5 border-2 border-stone-500 border-t-stone-50 rounded-full animate-spin"></span> : "Sign In"}
          </button>
        </form>

        <div className="mt-8 text-center text-sm font-sans text-stone-500">
          Don't have an account? <Link to="/register" className="text-stone-900 font-semibold hover:underline">Create one</Link>
        </div>
      </motion.div>
    </div>
  );
